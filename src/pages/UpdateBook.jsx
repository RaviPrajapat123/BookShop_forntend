...
function UpdateBook() {
 ...
 const submit = async () => {
  try {
    const response = await axios.put("http://localhost:3000/update-book", data,{headers});
  ...
  useEffect(() => {
    const fetch=async()=>{
        const respones=await axios.get(`http://localhost:3000/get-book-by-id/${id}` )
  ...
  return (
     <div className='bg-zinc-900 h-[100%]  p-0 md:p-4'>
         ...
         <div className='p-4 bg-zinc-800 rounded '>
            ...
            <button className='mt-4 bg-blue-500 text-white font-semibold py-2 rounded hover:bg-blue-600 transition-all duration-300 px-2 cursor-pointer' onClick={submit}>Update Book</button>
         </div>
    </div>
  )
}
